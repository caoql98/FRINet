...
class OneModel(nn.Module):
    ...
    def forward(self, x, s_x, s_y, y_m, y_b, cat_idx=None):
        ...
        if self.training:
            ...
            pred = pred.view(bs).float().mean(dim=0)
            loss_size1 = -self.focal_loss(pred)
            main_loss = main_loss + 0.5*loss_size1

            aux_loss1 = self.criterion(meta_out, y_m.long())

            out_softmax = F.softmax(meta_out, dim=1)
            pred = out_softmax[:,1,:,:]
            pred = pred.sum(dim=[1,2])/(w*h)
            pred = pred.view(bs).float().mean(dim=0)
            loss_size2 = -self.focal_loss(pred)
            aux_loss1 = aux_loss1 + 0.5*loss_size2

            aux_loss2 = self.criterion(base_out, y_b.long())

            out_softmax = F.softmax(base_out, dim=1)
            pred = out_softmax[:,1,:,:]
            pred = pred.sum(dim=[1,2])/(w*h)
            pred = pred.view(bs).float().mean(dim=0)
            loss_size3 = -self.focal_loss(pred)
            aux_loss2 = aux_loss2 + 0.5*loss_size3
