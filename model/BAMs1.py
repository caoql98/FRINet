...
class OneModel(nn.Module):
    def __init__(self, args, cls_type=None):
        ...
        self.weight_merge = nn.Conv2d(4, 4, kernel_size=1, bias=False)
        # self.weight_merge.weight = nn.Parameter(torch.tensor([[0.25],[0.25],[0.25],[0.25]]).reshape_as(self.weight_merge.weight))
    ...
    def get_optim(self, model, args, LR):
        if args.shot > 1:
            optimizer = torch.optim.SGD(
                ...
                {'params': model.weight_merge.parameters()},                  
    ...
    def forward(self, x, s_x, s_y, y_m, y_b, cat_idx=None):
        ...
        supp_pro1 = (weight_soft.permute(0,2,1,3) * supp_pro1).sum(2,True)
        supp_pro2 = (weight_soft.permute(0,2,1,3) * supp_pro2).sum(2,True)
        supp_pro3 = (weight_soft.permute(0,2,1,3) * supp_pro3).sum(2,True)

        supp_pro_all = torch.cat([supp_pro,supp_pro1,supp_pro2,supp_pro3],2) # [bs, 256, 4, 1]
        supp_pro_all = supp_pro_all.squeeze(-1)
        normalize_supp_pro_all = F.normalize(supp_pro_all, dim=1)# [bs, 256, 4]
        bq,cq,hq,wq = query_feat.shape
        normalize_query_feat = F.normalize(query_feat.reshape(bq,cq,hq*wq), dim=1)
        cosine_matrix = torch.bmm(normalize_supp_pro_all.permute(0,2,1), normalize_query_feat)# [bs, 4, hq*wq]
        cosine_matrix = self.weight_merge(cosine_matrix.reshape(bq,4,hq,wq)).reshape(bq,4,hq*wq)
